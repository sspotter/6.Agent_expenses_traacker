 ...
 *   → Paid on Jan 20, 2026
 */
export function withTimestamp(notes: string, amount: number, status: string, date: Date | string): string {
  const timestamp = format(new Date(), 'MMM d, h:mm a');
  const displayDate = typeof date === 'string' ? format(parseISO(date), 'MMM d, yyyy') : format(date, 'MMM d, yyyy');
  const cleanNotes = notes.trim() || 'No additional notes';
  
  const statusLabel = status === 'FULL' ? 'Full' : status === 'PARTIAL' ? 'Partial' : 'Missed';
