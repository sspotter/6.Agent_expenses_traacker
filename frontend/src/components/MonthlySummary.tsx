...
export const MonthlySummary: React.FC<MonthlySummaryProps> = ({   summary,
  ...
  const { chats, fetchChats } = useTelegramChats(workerId);
...
function generateFullCsv(
  ...
  return [
    ...
    link.href = url;
    link.setAttribute('download', `collection_summary_${workerName}_${summary.month}.csv`); // simplified
  ...
  try {
    ...
    const surplus =
      summary.total_collected > summary.total_expected
        ? summary.total_collected - summary.total_expected
        : 0;
    const paid_debt =  summary.income_debt;
//     const text = `
...
const test_pay= summary.settled_amount - summary.expense_debt;

const paidExpenses = expensePayments.reduce(
...
// const text = `$${test_pay.toLocaleString() || 0}`
const text = `
...
${
  isOverachieved
    ? `🎉 *Surplus Achieved!*  
+$${(summary.total_collected - summary.worked_expected).toLocaleString()} collected above expected`
    : ''
}

${
  summary.income_debt > 0 || summary.expense_debt > 0
    ? `
━━━━━━━━━━━━━━
⚠️ *Outstanding Breakdown*
━━━━━━━━━━━━━━
${summary.income_debt > 0 ? `• Income Gap: $${summary.income_debt}\n` : ''}
${summary.expense_debt > 0 ? `• Unpaid Expenses: $${summary.expense_debt}` : ''}
`
    : ''
}
`;

    // Prepare text (removed to save space, assuming it's the same)
    
    // Logic: Send to all active specific chats OR fallback to env
    const activeChats = chats.filter(c => c.is_active);

    if (activeChats.length > 0) {
      // Send to each specific chat
      const promises = activeChats.map(chat => sendTelegramMessage(text, chat.chat_id));
      await Promise.all(promises);
      setMessage(`✅ Summary sent to ${activeChats.length} chat(s)!`);
    } else {
      // Fallback to env (pass undefined as targetChatId)
      await sendTelegramMessage(text);
      setMessage('✅ Summary sent to default chat!');
    }
  } catch (err: any) {
    setMessage('❌ Error: ' + err.message);
  } finally {
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-6 mb-8">
            ...
            {/* Surplus Card - Per Spec Section 5 */}
            {summary.surplus_total > 0 && (
              <div className="shadow-xl hover:shadow-teal-500/50 glass p-5 rounded-2xl space-y-2 group hover:translate-y-[-2px] transition-premium border-2 border-teal-500/20">
                <p className="text-xs font-semibold text-teal-500 uppercase tracking-wider">Surplus (Over-Collections)</p>
                <div className="flex flex-col gap-1">
                  <p className="text-2xl font-bold text-teal-500 tracking-tight">
                    ${summary.surplus_total.toLocaleString()}
                  </p>
                  <div className="flex flex-wrap gap-1 mt-1">
                    <span className="text-[8px] font-black bg-teal-500/10 text-teal-400 px-1.5 py-0.5 rounded-md uppercase tracking-tighter">
                      ${summary.surplus_unallocated} Unallocated
                    </span>
                    {summary.surplus_total > summary.surplus_unallocated && (
                      <span className="text-[8px] font-black bg-success/10 text-success px-1.5 py-0.5 rounded-md uppercase tracking-tighter">
                        ${summary.surplus_total - summary.surplus_unallocated} Used
                      </span>
                    )}
                  </div>
                </div>
