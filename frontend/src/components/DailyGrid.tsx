...
export const DailyGrid: React.FC<DailyGridProps> = ({ 
  ...
  const getWeeklyFinancials = (weekDays: Date[]) => {
    ...
    // Income from settlement allocations targeted at days in this week
    const settlementIncome = weekDays.reduce((acc, day) => {
        const col = getCollection(day);
        if (!col) return acc;
        const dayAllocated = (allocations || [])
            .filter(a => a.collection_id === col.id)
            .reduce((sum, a) => sum + a.allocated_amount, 0);
        return acc + dayAllocated;
    }, 0);

    // Total Net Weekly Income (Direct collections + Late settlements)
      ...
      <div className="min-w-[850px] grid grid-cols-5 gap-6">
        {isLoading ? (
          ...
          weeks.map((weekDays, index) => {
              const { income, settlementIncome, expense, paidAmount, expenseObj } = getWeeklyFinancials(weekDays);
            ...
            <div key={index} className="flex flex-col space-y-4">
              ...
              <div className="space-y-3">
                {weekDays.map(day => {
                  ...
                  const expected = collection?.expected_amount ?? 0;
...
const totalEffectivePaid = collected + allocated;
                  const effectivePaid = collected + allocated;

                  const status = collection?.status;
                  const base_expected = getRateForDate(day);
                  const is_override = collection?.is_expected_override ?? (collection ? expected !== base_expected : false);
                  ...
                  const getAmountColor = (r?: DailyResolution) => {
  switch (r) {
  ...
  | 'PARTIAL_ZERO'     // 🔴
  | 'MISSED_NO_WORK';  // ⚪
const getResolutionStyles = (r?: DailyResolution, today?: boolean) => {
  if (r === 'PAID_ON_TIME') return 'bg-success/10 border-success/30';
  if (r === 'SETTLED_LATER') return 'bg-resolved/10 border-resolved/30';
  if (r === 'PARTIAL') return 'bg-warning/10 border-warning/30';
  if (r === 'PARTIAL_ZERO') return 'bg-danger/10 border-danger/30';
  if (r === 'MISSED_NO_WORK') return 'bg-black/60 bg-black/60 border-1 border-dashed border-gray-200 text-gray-400 shadow-md';

  return clsx(
    'bg-white border-gray-100 shadow-md text-gray-500 hover:bg-amber-500/10 transition-premium',
    today && 'ring-2 ring-primary/20'
  );
};
let resolution: DailyResolution | undefined = undefined;
...
/** 1️⃣ NO WORK DAY */
  ...
  resolution = 'MISSED_NO_WORK';
}

/** 2️⃣ PAID ON TIME */
else if (
  collected >= expected &&
  collected === totalEffectivePaid
) {
  resolution = 'PAID_ON_TIME';
}

/** 3️⃣ SETTLED LATER */
else if (totalEffectivePaid >= expected) {
  resolution = 'SETTLED_LATER';
}

/** 4️⃣ PARTIAL ZERO (worked, zero pay) */
else if (totalEffectivePaid === 0) {
  resolution = 'PARTIAL_ZERO';
}

/** 5️⃣ PARTIAL (1/100, 20/100 etc.) */
else {
  resolution = 'PARTIAL';
}
const displayPaid = Math.min(totalEffectivePaid, expected);
                   ...
                   return (
                     <div key={day.toString()} className="relative group">
                       <div
                        ...
                        //  )}
                        className={clsx(
  "w-full p-4 rounded-2xl border-2 transition-premium",
  getResolutionStyles(resolution, today),
  ...
  {resolution === 'PAID_ON_TIME' && <Check className="text-success" />}
  {resolution === 'SETTLED_LATER' && <Check className="text-resolved" />}
  {resolution === 'PARTIAL' && <AlertCircle className="text-[9px] font-bold text-warning uppercase" />}
  {resolution === 'PARTIAL_ZERO' && <AlertCircle className="text-[9px] font-bold text-danger uppercase" />}
  {resolution === 'MISSED_NO_WORK' && <X className="text-[9px] font-bold text-gray-400 uppercase" />}
                              {is_override && (
