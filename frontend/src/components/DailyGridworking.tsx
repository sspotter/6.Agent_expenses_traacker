...
const getResolutionStyles = (resolution: DailyResolution | null, today: boolean) => {
  if (!resolution)
    return clsx(
  ...
  switch (resolution) {
    ...
    case 'SETTLED_LATER':
      return 'bg-resolved/10 text-resolved border-resolved/30';
...
const totalEffectivePaid = collected + allocated;
                  ...
                  let resolution: DailyResolution | null = null;

if (!collection) {
  resolution = null; // untouched day → white card
}
else if (expected === 0) {
...
else if (collected === 0 && allocated === 0) {
