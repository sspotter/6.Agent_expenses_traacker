...
const getResolutionStyles = (r?: DailyResolution, today?: boolean) => {
  ...
  if (r === 'MISSED_NO_WORK') return 'bg-gray-100 border-gray-200 text-gray-400';
...
else if (expected === 0) resolution = 'MISSED_NO_WORK';
else if (collected >= expected && collected === totalEffectivePaid)
...
const displayPaid = Math.min(totalEffectivePaid, expected);
  ...
  {resolution === 'PARTIAL' && <AlertCircle className="text-warning" />}
  {resolution === 'PARTIAL_ZERO' && <AlertCircle className="text-danger" />}
  {resolution === 'MISSED_NO_WORK' && <X className="text-gray-400" />}
